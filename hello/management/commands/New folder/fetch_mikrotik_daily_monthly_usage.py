...
from hello.utils3 import calculate_daily_usage, calculate_monthly_usage, check_and_handle_quota()
import re
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if MOCK_MODE:
            ...
            resultsecond=calculate_monthly_usage() 
            self.stdout.write(str(resultsecond))
        ...
        try:
            with connect(host=host, username=username, password=password, port=port, use_ssl=use_ssl) as api:
                ...
                for source_name, path in sources:
                    ...
                    if not items:
                        self.stdout.write(self.style.NOTICE(f'No entries for {source_name}'))
                        continue
                    ...
                    for item in items:
                        ...
                        bytes_in = safe_int(d.get('bytes-in') or d.get('bytes_in') or d.get('rx-byte') or 0)
                        bytes_out = safe_int(d.get('bytes-out') or d.get('bytes_out') or d.get('tx-byte') or 0)
